...
def print_backend_status(device=None):
    ...
    scan_status = (
        "ON(cuda kernel)"
        if using_cuda and HAS_SELECTIVE_SCAN_CUDA
        else "OFF(fallback)"
    )
    conv_status = (
        "ON(cuda kernel)"
        if using_cuda and HAS_CAUSAL_CONV1D
        else "OFF(fallback)"
    )
    print(
...
class SSM_Manipulation(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_casual1D and x.is_cuda:
            x = causal_conv1d_fn(
                x=x,
...
class S6(nn.Module):
    ...
    def forward(self, u):
        ...
        delta_softplus = True
        dtype_in = u.dtype
        use_scan_cuda = self.use_scan_cuda and u.is_cuda
        if not use_scan_cuda:
            u = u.float()
            delta = delta.float()
