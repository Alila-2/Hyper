<template>
    <div class="spectrum-library-container">
        <h1 class="page-title">典型目标光谱库</h1>

        <!-- 新增/编辑光谱模态框 -->
        <el-dialog :visible.sync="editDialogVisible"
                   :title="isEditing ? '编辑材料数据' : '新增材料数据'"
                   width="600px"
                   :close-on-click-modal="false"
                   @closed="resetEditForm">
            <el-form :model="editForm" :rules="formRules" ref="editForm" label-width="100px">
                <el-form-item label="材料编号" prop="code">
                    <el-input v-model="editForm.code" placeholder="请输入材料编号，如：MAT-001"></el-input>
                </el-form-item>
                <el-form-item label="材料名称" prop="name">
                    <el-input v-model="editForm.name" placeholder="请输入材料名称"></el-input>
                </el-form-item>
                ...
                <el-form-item label="应用场景" prop="applications">
                    <el-input type="textarea"
                              v-model="editForm.applications"
                              placeholder="请输入应用场景"
                              :rows="2"></el-input>
                </el-form-item>
                <el-form-item label="测量条件" prop="conditions">
                    <el-input v-model="editForm.conditions" placeholder="如：室温25°C，标准光源"></el-input>
                </el-form-item>
                ...
                <el-form-item label="光谱数据" prop="spectrumDataFile">
                    <el-upload class="upload-demo"
                               action="#"
                               :auto-upload="false"
                    ...
                    <!-- 显示自动计算的波长范围 -->
                    <div v-if="autoWavelengthRange" class="wavelength-range-hint">
                        <el-tag type="info" size="small">自动计算波长范围: {{ autoWavelengthRange }}</el-tag>
                    </div>
                </el-form-item>
            </el-form>
            ...
            <div class="spectrum-detail-wrapper">
                ...
                <div class="spectrum-description">
                    ...
                    <div class="description-content">
                        ...
                        <div class="description-item">
                            <label>波长范围：</label>
                            <span>{{ currentSpectrum.wavelengthRange || '无' }}</span>
                        </div>
                        <div class="description-item">
                            <label>测量条件：</label>
        ...
        <div class="spectrum-list">
            <div v-for="spectrum in spectrums"
                ...
                <div class="spectrum-thumbnail-wrapper">
                        ...
                        <div class="action-buttons">
                            <el-tooltip content="编辑" placement="top">
                                <el-button circle
                                           ...
                                           type="primary"
                                           icon="el-icon-edit"
                                           @click.stop="editSpectrum(spectrum)"></el-button>
                            </el-tooltip>
                            <el-tooltip content="删除" placement="top">
                                <el-button circle
                                           ...
                                           type="danger"
                                           icon="el-icon-delete"
                                           @click.stop="deleteSpectrum(spectrum)"></el-button>
                            </el-tooltip>
                        </div>

                        <div class="view-icon" @click.stop="viewDetail(spectrum)">
                    ...
                    <div class="spectrum-info">
                        ...
                        <div class="spectrum-wavelength-range">
                            波长范围: {{ spectrum.wavelengthRange }}
                        </div>
            ...
            <!-- 新增按钮卡片 -->
            <div class="spectrum-item add-item" @click="addSpectrum">
                <div class="spectrum-thumbnail-wrapper">
                    <div class="spectrum-thumbnail add-thumbnail">
                        <div class="add-icon">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 编辑材料
            async editSpectrum(spectrum) {
                this.isEditing = true;
                this.editForm = {
                    id: spectrum.id,
                    name: spectrum.name,
                    code: spectrum.code,
                    brief: spectrum.brief || '',
                    description: spectrum.description || '',
                    applications: spectrum.applications || '',
                    conditions: spectrum.conditions || ''
                };
                this.thumbnailFileList = [];
                this.spectrumFileList = [];
                this.autoWavelengthRange = spectrum.wavelengthRange || '无';
                this.editDialogVisible = true;
            },

            // 删除材料
            deleteSpectrum(spectrum) {
                this.$confirm(`确定要删除材料 "${spectrum.name}" 吗？`, '提示', {
                    ...
                    try {
                        ...
                        formData.append('id', spectrum.id);

                        const response = await axios.post(`${this.config.apiBaseUrl}/delete_data`, formData, {
                            headers: {
            ...
            // 从光谱数据文件计算波长范围
            calculateWavelengthRangeFromFile(file) {
                return new Promise((resolve) => {
                    const reader = new FileReader();
                    reader.onload = (e) => {
                        try {
                            const spectrumData = JSON.parse(e.target.result);
                            console.log('解析的光谱数据:', spectrumData);

                            const calculatedRange = this.calculateWavelengthRangeFromData(spectrumData);
                            resolve(calculatedRange);
                        } catch (error) {
                            console.error('解析光谱数据文件失败:', error);
                            resolve('无');
                        }
                    };
                    reader.onerror = () => {
            ...
            // 从光谱数据计算波长范围（用于现有数据）
            calculateWavelengthRangeFromData(spectrumData) {
                try {
                    // 支持两种格式：{data: [...]} 或直接是数组
                    let dataArray = [];
                    if (spectrumData && spectrumData.data && Array.isArray(spectrumData.data)) {
                        dataArray = spectrumData.data;
                    } else if (Array.isArray(spectrumData)) {
                        dataArray = spectrumData;
                    }

                    if (dataArray.length > 0) {
                        // 提取所有波长值
                        const wavelengths = dataArray.map(point => {
                            // 确保有wavelength字段
                            if (point && typeof point === 'object' && 'wavelength' in point) {
                                return parseFloat(point.wavelength);
                            }
                            return null;
                        }).filter(w => w !== null); // 过滤掉无效值

                        if (wavelengths.length > 0) {
                            const minWavelength = Math.min(...wavelengths);
                            const maxWavelength = Math.max(...wavelengths);
                            const wavelengthRange = `${minWavelength.toFixed(0)}-${maxWavelength.toFixed(0)} nm`;
                            console.log('从数据计算出的波长范围:', wavelengthRange);
                            return wavelengthRange;
                        } else {
                            console.warn('光谱数据中没有有效的波长值');
                            return '无';
                        }
                    } else {
                        console.warn('光谱数据为空或格式不正确');
                        return '无';
                    }
                } catch (error) {
                    console.error('从数据计算波长范围失败:', error);
                    return '无';
                }
            },
            ...
            async submitForm() {
                this.$refs.editForm.validate(async (valid) => {
                    ...
                    try {
                        ...
                        // 处理波长范围 - 从前端计算
                        let wavelengthRange = '无';
                        if (this.spectrumFileList.length > 0) {
                            wavelengthRange = await this.calculateWavelengthRangeFromFile(this.spectrumFileList[0].raw);
                        }
                        formData.append('wavelengthRange', wavelengthRange);

                        // 添加文件
                        if (this.thumbnailFileList.length > 0) {
                            formData.append('thumbnail', this.thumbnailFileList[0].raw);
                        }
                        if (this.spectrumFileList.length > 0) {
                            formData.append('spectrumDataFile', this.spectrumFileList[0].raw);
                        }

                        const url = this.isEditing ?
                            `${this.config.apiBaseUrl}/mod_data` :
                            `${this.config.apiBaseUrl}/add_data`;
                        ...
                        if (error.response && error.response.data && error.response.data.detail) {
                            this.$message.error('操作失败: ' + error.response.data.detail);
                        } else {
            ...
            handleThumbnailChange(file, fileList) {
                this.thumbnailFileList = fileList.slice(-1); // 只保留最后一个文件
            },

            // 处理光谱数据文件变化
            async handleSpectrumFileChange(file, fileList) {
                this.spectrumFileList = fileList.slice(-1); // 只保留最后一个文件

                // 自动计算并显示波长范围
                if (file.raw) {
                    this.autoWavelengthRange = '计算中...';
                    const range = await this.calculateWavelengthRangeFromFile(file.raw);
                    this.autoWavelengthRange = range;
                } else {
                    this.autoWavelengthRange = '';
                }
            },
            ...
            createBasicSpectrumItem(item) {
                ...
                return {
                    ...
                    wavelengthRange: '计算中...', // 初始显示"计算中..."，加载数据后更新
                    ...
                    dataLoaded: false,
                    loadingData: false,
                    wavelengthCalculated: false // 标记波长范围是否已计算
                };
            },
                ...
                try {
                    ...
                    if (spectrum.spectrumDataFile) {
                        ...
                        if (spectrumData && spectrumData.length > 0) {
                            const calculatedRange = this.calculateWavelengthRangeFromData(spectrumData);
                            spectrum.wavelengthRange = calculatedRange;
                            spectrum.wavelengthCalculated = true;
                            console.log(`自动更新波长范围: ${spectrum.name} -> ${calculatedRange}`);
                        }
                    }
                    ...
                    if (spectrumData && spectrumData.length > 0) {
                        ...
                        // 如果没有光谱数据，设置波长范围为"无"
                        if (!spectrum.wavelengthCalculated) {
                            spectrum.wavelengthRange = '无';
                        }
                    }
            ...
            renderDetailChart() {
                this.$nextTick(() => {
                    ...
                    // 计算实际波长范围（用于横坐标映射）
                    const wavelengthRange = dataRange.maxWavelength - dataRange.minWavelength;
                    const reflectanceRange = dataRange.maxReflectance - dataRange.minReflectance;
                    ...
                    mainData.forEach((point, index) => {
                        // 修复横坐标映射：使用正确的波长范围变量
                        const x = padding.left + ((point.wavelength - dataRange.minWavelength) / wavelengthRange) * chartWidth;
                        const y = padding.top + chartHeight - ((point.reflectance - dataRange.minReflectance) / reflectanceRange) * chartHeight;
...
<style scoped>
    ...
    .spectrum-item:hover .action-buttons {
        opacity: 1;
    }
