  ...
  methods: {
    ...
    async loadVideos() {
      try {
        ...
        // 尝试多个可能的JSON文件路径
        const possiblePaths = [
          `${this.basePath}/video-listC.json`,
          './static/video-listC.json',
          '/static/video-listC.json',
          'static/video-listC.json'
        ];
    ...
    getVideoUrl(fileName) {
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        if (typeof window !== 'undefined' && window.require) {
          try {
            ...
            if (electronApp) {
              // Electron环境
              const appPath = electronApp.getAppPath();
              videoPath = `file://${path.join(appPath, '../static/videosLib/C', fileName).replace(/\\/g, '/')}`;
              return videoPath;
            }
          } catch (e) {
        ...
        videoPath = `${baseUrl}/static/videosLib/C/${fileName}`;
      } else {
        // 开发环境
        const baseUrl = process.env.BASE_URL || '';
        videoPath = `${baseUrl}/static/videosLib/C/${fileName}`;
