<template>
  <div class="video-library-container">
    <h1 class="page-title">高分辨率视频库</h1>
  ...
  methods: {
    ...
    async loadVideos() {
      try {
        ...
        // 尝试多个可能的JSON文件路径
        const possiblePaths = [
          `${this.basePath}/video-listB.json`,
          './static/video-listB.json',
          '/static/video-listB.json',
          'static/video-listB.json'
        ];
        ...
        if (!response) {
          throw new Error('无法找到视频列表文件，请检查 video-listB.json 是否存在');
        }
    ...
    getVideoUrl(fileName) {
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        if (typeof window !== 'undefined' && window.require) {
          try {
            ...
            if (electronApp) {
              // Electron环境
              const appPath = electronApp.getAppPath();
              videoPath = `file://${path.join(appPath, '../static/videosLib/B', fileName).replace(/\\/g, '/')}`;
              return videoPath;
            }
          } catch (e) {
        ...
        videoPath = `${baseUrl}/static/videosLib/B/${fileName}`;
      } else {
        // 开发环境
        const baseUrl = process.env.BASE_URL || '';
        videoPath = `${baseUrl}/static/videosLib/B/${fileName}`;
