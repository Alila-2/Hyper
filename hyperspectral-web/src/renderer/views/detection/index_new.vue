          ...
          <div class="block3 active">
            ...
            <div class="metrics-container">
              <el-card class="metric-card">
                ...
                <div class="metric-value">{{ currentFPS.toFixed(1) }} FPS</div>
              ...
              <el-card class="metric-card">
                ...
                <div class="metric-value">{{ detectionConfidence.toFixed(3) }}</div>
  ...
  methods: {
    ...
    async getStaticBasePath() {
      ...
      if (!isElectron) {
        ...
        return `${window.location.origin}/static/videos_811/${this.selectedRange}/`;
      }
          ...
          if (window.require) {
            ...
            try {
             ...
             // return `file://${staticPath.replace(/\\/g, '/')}/videos/${this.selectedRange}/`;
              ...
              // return `file://${staticPath.replace(/\\/g, '/')}/videos_811/${this.selectedRange}/`;
            } catch (ipcError) {
    ...
    updateFPSValue() {
      ...
      // 根据选择的范围设置固定FPS或使用原来的逻辑
      if (this.fixedMetrics[this.selectedRange]) {
        this.currentFPS = this.fixedMetrics[this.selectedRange].fps;
      } else {
        // 原来的逻辑，用于仿真模式
    ...
    updateDetectionMetrics() {
      if (this.fixedMetrics[this.selectedRange]) {
        // 使用固定指标
        const metrics = this.fixedMetrics[this.selectedRange];
        this.detectionConfidence = metrics.confidence;
        this.currentMS = metrics.responseTime;
        this.currentFPS = metrics.fps;
      } else {
        // 原来的随机逻辑，用于仿真模式
        let min, max;
        if (this.selectedRange === "fz1") {
          min = 91;
          max = 93;
        ...
        this.detectionConfidence = +(Math.random() * (max - min) + min).toFixed(2);
        
        // 响应时间根据FPS计算
        this.currentMS = this.currentFPS > 0 ? +((1 / this.currentFPS) * 1000).toFixed(2) - 5 : 0;
      }
    },
    ...
    async onRangeChange() {
      ...
      try {
        ...
        // const fallbackBase = `${window.location.origin}/static/videos_811/${this.selectedRange}/`;
