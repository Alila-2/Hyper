    ...
    <div class="introduction-container">
      ...
      <div class="viewer">
        ...
        <div class="asset-container left-panel">
          ...
          <div>
            <h3 class="asset-title">原始图像</h3>
            <div class="image-info">
              <p class="info-text">
                {{ originalImageUrl ? '显示原始PNG图像' : '等待加载原始图像数据' }}
              </p>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    onImageLoad() {
      ...
      console.log('Pseudo color image loaded:', {
    ...
    onMouseMove(event) {
      if (!this.pseudoColorImageUrl || !this.isDataReady || !this.imageLoaded) return
      ...
      // 转换为图像原始坐标
      const x = Math.floor((relativeX / imgRect.width) * this.imageWidth)
      const y = Math.floor((relativeY / imgRect.height) * this.imageHeight)
      
      const validX = Math.max(0, Math.min(x, this.imageWidth - 1))
      const validY = Math.max(0, Math.min(y, this.imageHeight - 1))
...
.introduction-container {
  ...
  padding: 30px;*/
  width: 100%;  
/*background: rgba(255, 255, 255, 0.9); /* 增加白色背景透明度，让背景图隐约可见 */  
/*border-radius: 20px;  
box-shadow: 0 20px 40px rgba(0, 0, 0, 0.1);  
backdrop-filter: blur(10px);  
padding: 30px;  
max-width: 1800px; /* 可根据需要添加最大宽度限制 */
}
