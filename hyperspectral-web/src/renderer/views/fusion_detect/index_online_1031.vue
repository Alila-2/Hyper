    ...
    export default {
        ...
        data() {
            return {
                ...
                targetStatistics: {},         // { [type]: { object_count, sbl } }
                jsml: null,                   // 军事目标数
                wzml: null,                   // 伪装目标数
                pyl: null,                    // 伪装目标破译率（可选）
        ...
        computed: {
            ...
            allTargets() {
                ...
                this.materials.forEach(function (m) {
                    if (m.targets && Array.isArray(m.targets)) {
                        m.targets.forEach(function (t) { set.add(t) })
                    }
                })
                return Array.from(set)
            },
            // 总目标数 = 各类别 object_count 之和（无可选链）
            totalObjectsCount() {
                if (!this.targetStatistics) return 0
                var values = Object.values(this.targetStatistics)
                return values.reduce(function (sum, stats) {
            ...
            detectedTargetsWithCount() {
                ...
                return this.detectedTargets.map((t) => {
                    var stats = this.targetStatistics[t.type]
            ...
            async runAutoFusionDetect() {
                ...
                try {
                    ...
                    if (response.data && response.data.success) {
                        ...
                        this.jsml = (typeof response.data.jsml !== 'undefined') ? response.data.jsml : null
                        this.wzml = (typeof response.data.wzml !== 'undefined') ? response.data.wzml : null
                        ...
                        // 统计信息
                        this.detectionInfo = {
                            processingTime: (typeof response.data.runtime !== 'undefined' && response.data.runtime !== null) ? response.data.runtime : ((Date.now() - this.processStartTime) / 1000).toFixed(2),
                            materialCount: this.allMaterialNames.length,
                            targetCount: this.allTargets.length
                        }

                        this.debugInfo += '检测完成：发现 ' + this.detectedTargets.length + ' 种材料，总计 ' + this.totalObjectsCount + ' 个目标。\n'
